...
func TestAttributeManager_DecryptAttribute(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				appCode: "",
				referer: "gclid\\u003dCjwKCAjwvrOpBhBdEiwAR58-3JbBKDSOR9f08qFC5zsM_O-4Nvoqr_21kodMn0mnLcYj5d5q-P7dwBoCn68QAvD_BwE",
			},
			want: &AttributeInfo{
				Channel: "google",
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("AttributeManager.DecryptAttribute() = %v, want %v", got, tt.want)
			}
		})
	}
}
